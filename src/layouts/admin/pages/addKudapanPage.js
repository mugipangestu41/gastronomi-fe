...
export default function AddKudapanPage() {
  ...
  const [kudapans, setKudapan] = useState('')
  ...
  const handleUpload = async () => {
    ...
    try {
        ...
        await axios.post(`${API_URL}kudapan/createOneKudapan`,
        formData, {headers, 
        ...
        .then(() => {
          setErrCreate(`Success data kudapan ${kudapans} ditambahkan`)
          // window.alert(`Success data kudapan ${kudapans} ditambahkan`)
          setKudapan(null)
          setContent('')
          ...
          document.getElementById("myform").reset();
            navigate(`/admin/addKudapanPage/${wKecamatan}/${wIdKecamatan}`)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if(kudapans == null){
        ...
        formData.delete("tipe_makanan")
        formData.delete("alamat")
        setErrCreate("Kolom kudapan wajib di isi")
        setKudapan(null)
        setContent('')
        document.getElementById("myform").reset();
        ...
        formData.append("tipe_makanan", "kudapan")
        formData.append("content", contents)
        formData.append("alamat", alamatMakanans)
        formData.append("nama_makanan", kudapans)
        handleUpload()
      }
    ...
    <>
      ...
      <Container maxWidth="md"> 
        ...
        <Grid item xs={12} sm={12} md={12}>
          ...
          &nbsp;<a style={{color:"black"}}>Tambah Kudapan</a> 
          </p>
        </Grid>
        :
        <></>
          ...
          <Grid item xs={12} sm={12} md={12}>
            <h5>{errCreate}</h5>
            <h3>Tambah Kudapan di Kecamatan {wKecamatan}</h3>
            <form id='myform'>
            <Stack spacing={2} m={2}>
              
              <div className="form-group">
              <TextField id="kudapan" label="Nama Kudapan" variant="outlined" onChange={(e) => setKudapan(e.target.value)} fullWidth required/>
              </div>
